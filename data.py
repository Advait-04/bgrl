...
def get_dataset():
    ...
    dvalues_range2 = np.arange(41, 196)
    dopamine_values = np.concatenate((dvalues_range1, dvalues_range2))
    ...
    for i, dvalue in enumerate(dopamine_values):
        ...
        elif dvalue >= 40 and dvalue <= 196:
            acetyl_values[i] = np.random.randint(1, 3)  # Generate random values between 1 and 4

    # Levodopa values
    lvalues_range1 = np.arange(0, 101)
    lvalues_range2 = np.arange(100, 251)
    levodopa_values = np.concatenate((lvalues_range1, lvalues_range2))
    levodopa_values = np.tile(levodopa_values, common_length)[:common_length]

    return [
        dopamine_values, acetyl_values, levodopa_values
    ]
